		...
		if(isServerWorld){
			...
			if(!hasRedstoneSignal()){
				...
				if(timeUntilNextPump == 0 && getTank().getFluidAmount() <= 0){
					...
					while(target.getY() > 0 && w.getBlockState(target).getBlock() == com.mcmoddev.electricadvantage.init.Blocks.pump_pipe_electric){
						target = target.down();
					}
					if(target.getY() > 0 && w.isAirBlock(target) && getEnergy() >= ENERGY_COST_PIPE){
						// place pipe
						w.setBlockState(target, com.mcmoddev.electricadvantage.init.Blocks.pump_pipe_electric.getDefaultState());
